...
int P1_main(int argc, char* argv[])
{
	...
	while (1)
	{
		...
		//Parse input
		{
			static char* currsp, *lastsp,* qb,* qe,* myArgv[MAX_ARGS];
			newArgc = 0;
			myArgv[0] = currsp = lastsp = inBuffer;				// point to input string
			int bufferLen = strlen(inBuffer);
			bool inQuote = FALSE;
			for (int i = 0; i < bufferLen; i++) {
				//printf("\nchar:%c", currsp[0]);
				if (!inQuote && currsp[0] == ' ') {
					*currsp = 0;
					myArgv[newArgc++] = lastsp;
					//printf("\nlastsp:%s", lastsp);
					lastsp = currsp+1;
				}
				else if (!inQuote) {
					currsp[0] = tolower(currsp[0]);
				}
				if (currsp[0] == '\"') {
					if (!inQuote) { //beginning quote
						inQuote = TRUE;
					}
					else { //end quote found
						inQuote = FALSE;
						*++currsp = 0;
						myArgv[newArgc++] = lastsp;
						//printf("\nlastsp:%s", lastsp);
		...
		for (found = i = 0; i < NUM_COMMANDS; i++)
		{
			...
			{
				if (backgroundTask == TRUE) {
					//printf("create background task");
					createTask(newArgv[0], (*commands[i]->func), tcb[curTask].priority, newArgc, newArgv);
					found = TRUE;
					break;
				}
				else {
...
Command** P1_init()
{
	...
	// system
	commands[i++] = newCommand("add", "a", P1_add, "Add");
	commands[i++] = newCommand("args", "arg", P1_args, "Args");
